...
install_xenial:
	sudo apt-get install -y --no-install-recommends linux-image-extra-$(shell uname -r) linux-image-extra-virtual apt-transport-https \
	   	ca-certificates curl software-properties-common git wget python2.7 python-dev python-pip gnupg
	curl -fsSL https://apt.dockerproject.org/gpg | sudo apt-key add -
	sudo add-apt-repository "deb https://apt.dockerproject.org/repo/ ubuntu-$(shell lsb_release -cs) main"
	sudo apt-get update
